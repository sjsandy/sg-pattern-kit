			...
			if ( match && (match[1] || !context) ) {
				...
				// HANDLE: $(#id)
					...
					if ( elem && elem.parentNode ) {
						...
						// Otherwise, we inject the elements directly into the jQuery object
						this.length = 1;
						this[0] = elem;
					}
...
Expr = Sizzle.selectors = {
	...
	pseudos: {
		...
		"empty": function( elem ) {
			...
			// :empty is only affected by elements nodes and content nodes(including text(3), cdata(4)),
...
function matcherFromGroupMatchers( elementMatchers, setMatchers ) {
	// A counter to specify which elements is currently being matched
...
jQuery.support = (function( support ) {
	...
	// This requires a wrapper elements in IE
	...
	// Make sure that elements opacity exists
	...
	// Makes sure cloning an html5 elements does not cause problems
	...
	jQuery(function() {
		...
		// determining if an elements has been hidden directly using
		// display:none (it is still safe to use offsets if a parent elements is
...
function internalData( elem, name, data, pvt /* Internal Use Only */ ){
	...
	if ( !id ) {
		// Only DOM nodes need a new unique ID for each elements since their data
...
jQuery.extend({
	...
	acceptData: function( elem ) {
		// Do not set data on non-elements because it will not be cleared (#8335).
		...
		while ( t-- ) {
			...
			if ( !(handlers = events[ type ]) ) {
				...
				if ( !special.setup || special.setup.call( elem, data, namespaces, eventHandle ) === false ) {
					// Bind the global event handler to the elements
			...
			if ( (!special._default || special._default.apply( eventPath.pop(), data ) === false) &&
				...
				if ( ontype && elem[ type ] && !jQuery.isWindow( elem ) ) {
					...
					try {
						...
						// IE<9 dies on focus/blur to hidden elements (#1486,#12518)
			...
			trigger: function() {
				if ( this !== safeActiveElement() && this.focus ) {
					try {
						...
						// Support: IE<9
						// If we error on focus to hidden elements (#1486, #12518),
...
jQuery.removeEvent = document.removeEventListener ?
	...
	function( elem, type, handle ) {
		...
		if ( elem.detachEvent ) {
			...
			// detachEvent needed property on elements, by name of that event, to properly expose it to GC
...
jQuery.fn.extend({
	...
	// Determine the position of an elements within
...
function fixCloneNodeIssues( src, dest ) {
	...
	// IE10 throws NoModificationAllowedError if parent is null, #12132.
		...
		// elements in IE9, the outerHTML strategy above is not sufficient.
		...
		// or radio button. Worse, IE6-7 fail to give the cloned elements
...
jQuery.extend({
	clone: function( elem, dataAndEvents, deepDataAndEvents ) {
		...
		// IE<=8 does not properly clone detached, unknown elements nodes
		} else {
...
if ( window.getComputedStyle ) {
	...
	curCSS = function( elem, name, _computed ) {
		...
		// but not position css attributes, as those are proportional to the parent elements instead
	...
	if ( !jQuery.support.reliableMarginRight ) {
		jQuery.cssHooks.marginRight = {
			get: function( elem, computed ) {
				if ( computed ) {
					// WebKit Bug 13343 - getComputedStyle returns wrong value for margin-right
					// Work around by temporarily setting elements display to inline-block
...
try {
	...
	// Use the href attribute of an A elements
...
jQuery.fn.extend({
	...
	position: function() {
		...
		// note: when an elements has margin: auto the offsetLeft and marginLeft
